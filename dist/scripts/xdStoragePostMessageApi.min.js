...
(function(){function b(b,a){var c=XdUtils.extend(a,g);c.id=b;parent.postMessage(JSON.stringify(c),"*")}function f(c){var a;try{a=JSON.parse(c.data)}catch(e){}a&&"cross-domain-local-message"===a.namespace&&("local"==a.type&&("set"===a.action?m.setData(a.id,a.key,a.value):"get"===a.action?m.getData(a.id,a.key):"remove"===a.action?m.removeData(a.id,a.key):"key"===a.action?m.getKey(a.id,a.key):"size"===a.action?m.getSize(a.id):"length"===a.action?m.getLength(a.id):"clear"===a.action&&m.clear(a.id)),"session"==
a.type&&("set"===a.action?k.setData(a.id,a.key,a.value):"get"===a.action?k.getData(a.id,a.key):"remove"===a.action?k.removeData(a.id,a.key):"key"===a.action?k.getKey(a.id,a.key):"size"===a.action?k.getSize(a.id):"length"===a.action?k.getLength(a.id):"clear"===a.action?k.clear(a.id):"sync"===a.action?k.syncManual(a.id,a.key):"option"===a.action&&"auto-sync"==a.key&&(h=a.value,b(a.id,{}))))}function d(c){if(null!=c.newValue&&"cross-domain-storage-sync"==c.key){var a;try{a=JSON.parse(c.newValue)}catch(p){return}if("request"==
...
sessionStorage.removeItem(a.keys[c]):sessionStorage.setItem(a.keys[c],a.hash[a.keys[c]]);a.id?(0==n&&(n=(new Date).getTime()),l[a.id]&&(clearTimeout(l[a.id]),delete l[a.id]),b(a.id,{status:0,message:"sync completed"})):"auto"==a.method&&b(null,{status:0,keys:a.keys,hash:a.hash})}else a.id&&(0==n&&(n=(new Date).getTime()),l[a.id]&&(clearTimeout(l[a.id]),delete l[a.id]),b(a.id,{status:2,message:"nothing to sync  "+JSON.stringify(a)}))}}var g={namespace:"cross-domain-local-message"},h=!1,n=0,l={},m=
{getData:function(c,a){var e=localStorage.getItem(a);b(c,{key:a,value:e})},setData:function(c,a,e){localStorage.setItem(a,e);a={success:localStorage.getItem(a)===e};b(c,a)},removeData:function(c,a){localStorage.removeItem(a);b(c,{})},getKey:function(c,a){var e=localStorage.key(a);b(c,{key:e})},getSize:function(c){var a=JSON.stringify(localStorage).length;b(c,{size:a})},getLength:function(c){b(c,{length:localStorage.length})},clear:function(c){localStorage.clear();b(c,{})}},k={getData:function(c,a){var e=
sessionStorage.getItem(a);b(c,{key:a,value:e})},setData:function(c,a,e){sessionStorage.setItem(a,e);var d={success:sessionStorage.getItem(a)===e};h&&d.success&&k.syncAuto(a,e);b(c,d)},removeData:function(c,a){sessionStorage.removeItem(a);h&&k.syncAuto(a,null);b(c,{})},getKey:function(c,a){var e=sessionStorage.key(a);b(c,{key:e})},getSize:function(c){var a=JSON.stringify(sessionStorage).length;b(c,{size:a})},getLength:function(c){b(c,{length:sessionStorage.length})},clear:function(c){h&&k.syncAuto(null,
null);sessionStorage.clear();b(c,{})},syncManual:function(c,a){var e={type:"request",method:"manual",id:c,keys:a,time:(new Date).getTime()};try{localStorage.setItem("cross-domain-storage-sync",JSON.stringify(e)),localStorage.removeItem("cross-domain-storage-sync")}catch(p){b(c,{status:1,message:p});return}l[c]=setTimeout(function(){delete l[c];b(c,{status:1,message:"timed out"})},1E3)},syncAuto:function(b,a){var c,d={type:"response",method:"auto",id:null,keys:[],hash:{}};if(null!=b)d.keys.push(b),
d.hash[b]=a;else{var f;for(c=0;c<sessionStorage.length;c++)f=sessionStorage.key(c),d.keys.push(f),d.hash[f]=null}localStorage.setItem("cross-domain-storage-sync",JSON.stringify(d));localStorage.removeItem("cross-domain-storage-sync")}};window.addEventListener?(window.addEventListener("message",f,!1),window.addEventListener("storage",d,!1)):(window.attachEvent("onmessage",f),window.attachEvent("onstorage",d));parent.postMessage(JSON.stringify({namespace:"cross-domain-local-message",id:"iframe-ready"}),
"*")})();
